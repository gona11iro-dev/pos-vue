...
- `dist-electron/POS Tio Ale-0.1.2-linux-x64.tar.gz`: bundle portable con `install.sh`
- `dist-electron/POS Tio Ale-0.1.2-amd64.deb`: paquete para Debian, Ubuntu, Kali y derivadas

Uso:

```sh
cd dist-electron
tar -xzf "POS Tio Ale-0.1.2-linux-x64.tar.gz"
cd "POS Tio Ale-0.1.2-linux-x64"
./install.sh
```
...
```sh
sudo apt install ./dist-electron/POS\ Tio\ Ale-0.1.2-amd64.deb
```
